...
public class TeleportPoint : MonoBehaviour
{
    ...
    public void TeleportPointSettings(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            StartCoroutine(SceneTransitionCoroutine(destinationScene, other));
        }
    }
    IEnumerator SceneTransitionCoroutine(SceneController.GameScenes scene, Collider other)
    {
        UIManager.Instance.sceneTransitionAnimator.SetTrigger("start");

        yield return new WaitForSeconds(1.5f); // Yükleme öncesi kýsa bir gecikme
