...
public class EnemyAudioManager : MonoBehaviour
{
    ...
    public AudioClip[] terrainLayerSounds;
    ...
    public void PlayFootstepSound()
    {
        ...
        if (Physics.Raycast(rayStart, Vector3.down, out RaycastHit hit, rayDistance))
        {
            ...
            if (terrain != null)
            {
                ...
                if (terrainTextureIndex >= 0 && terrainTextureIndex < terrainLayerSounds.Length)
                {
                    clipToPlay = terrainLayerSounds[terrainTextureIndex];
                }
            }
        ...
        if (clipToPlay != null)
        {
            enemyAudioSource.pitch = Random.Range(0.9f, 1.1f); // Hafif kalýnlýk/incelik deðiþimi robotikliði önler
    ...
    //Animation Event
    public void PlayAttackSound() { if (attackSound != null) enemyAudioSource.PlayOneShot(attackSound); }

    //Kod bazli
    public void PlayHurtSound() { if (hurtSound != null) enemyAudioSource.PlayOneShot(hurtSound); }
    //Kod bazli
    public void PlayBlockSuccessSound() { if (blockSuccessSound != null) enemyAudioSource.PlayOneShot(blockSuccessSound); }
}